...
3. Prepare the materials required by the script:
   1. Download the live CLS items spreadsheet and save it as `cls-active-items.csv` in the same directory as the script.
   2. Download the archived CLS items spreadsheet and save it as `cls-archived-items.csv` in the same directory as the script.
4. Start running the script by creating a separate terminal window and running `npm run start`.
5. Items that should be archived will be moved into the archived collection.
6. Halfway through, the script will prompt you to download the product and label images. Follow these steps:
...
7. The script will continue running to upload the images to the S3 bucket (this will use the `isomer-production` AWS profile automatically, adjust the configuration variable at the top of the index.js file if you use a different name), and then subsequently create the new CLS items.
8. At the end, the script will generate a new file called `cls-redirection-mappings.csv`. Add the contents of this file into the list of redirects inside our `isomer-next-infra` repo, then perform a `pulumi up`.
9. Verify that the new items have been created successfully on the production Isomer Studio.
