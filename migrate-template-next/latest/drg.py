...
# This is the DRG updated body parts CSV file
DRG_BODY_PARTS_CSV = "drg-body-parts.csv"
...
def create_drg_page(drg_code, drg_records, drg_by_hospital, drg_body_parts):
  # Step 1: Create the page description
  # Take the first record to extract the common information
  first_record = drg_records[0]

  # Get the updated body parts from latest csv, else take the old one (for removed DRG codes)
  try:
    body_parts = [str(part).strip() for part in [drg_body_parts[0]['Body Part 1'], drg_body_parts[0]['Body Part 2']] if part != '']
  except:
  ...
  output_file = f"{OUTPUT_DIRECTORY}/drg-{drg_code.replace(">", "more-than-").replace("≤", "less-than-")}-bill-information.json".lower().replace("_", "-")
  output = {
    "version": "0.1.0",
    "page": {
...
def main():
  ...
  body_parts_df = pd.read_csv(DRG_BODY_PARTS_CSV)
  body_parts = body_parts_df.fillna('').to_dict(orient='records')
  ...
  for drg_code in drg_codes:
    ...
    drg_body_parts = [record for record in body_parts if record['DRG'] == drg_code]
    
    # Create the TOSP page
    create_drg_page(drg_code, drg_records, drg_by_hospital, drg_body_parts)
