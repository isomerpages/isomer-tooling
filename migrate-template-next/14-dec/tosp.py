...
def create_tosp_page(tosp_code, tosp_records, tosp_by_hospital, surg_ann_records, hosp_records):
  ...
  for body_part in body_parts:
    output_file = f"{OUTPUT_DIRECTORY}/tosp-{tosp_code.replace(">", "more-than-").replace("≤", "less-than-").replace("<=", "less-than-").replace(">=", "more-than-")}-{body_part.replace("/", "-").replace(" ", "-").replace(",", "")}-bill-information.json".lower().replace("_", "-")
    output = {
      "version": "0.1.0",
      "page": {
